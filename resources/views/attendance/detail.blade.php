...
/** 表示する出退勤（申請中があればそちら優先） */
$displayStartTime = old('start_time',
    $fixRequest?->start_time
        ? Carbon::parse($fixRequest->start_time)->format('H:i')
        : $attendance->display_start_time
);

$displayEndTime = old('end_time',
    $fixRequest?->end_time
        ? Carbon::parse($fixRequest->end_time)->format('H:i')
        ...
        <tr>
            ...
            <td>
                <textarea
                    ...
                    {{ (!$isAdmin && $isPending) ? 'disabled' : '' }}
                >{{ old('remark', $fixRequest?->remark ?? $attendance->remark ?? '') }}</textarea>
            </td>
        </tr>
    </table>
